...
function getFibonacciNumber(index) {
  return Math.trunc(
    ((1 + Math.sqrt(5)) ** index - (1 - Math.sqrt(5)) ** index) /
      (2 ** index * Math.sqrt(5))
  );
}
...
function isSafeInteger(number) {
  if (Number.isSafeInteger(number) && number === Math.trunc(number)) {
    return true;
  }
  return false;
}
 ...
 * -1, 1 => -1 | 0 | 1
 */
function getRandomInteger(minF, max) {
  return Math.round(Math.random() * (max - minF) + minF);
}
...
function getCountOfOddNumbers(number) {
  let result = 0;
  for (let i = 1; i <= Math.sqrt(number ** 2); ) {
    if (i % 2 === 1) {
      result += 1;
    }
    i += 1;
